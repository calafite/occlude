...
namespace {
inline std::byte castChar(unsigned char character) {
  return static_cast<std::byte>(character);
}

inline unsigned char castByte(std::byte byte) {
...
Hash Hash::fromFile(const FilePath &path) {
  ...
  picosha2::hash256(std::istreambuf_iterator<char>(in),
                    std::istreambuf_iterator<char>(), digest.begin(),
                    digest.end());
  ...
  std::ranges::transform(digest, rBytes.begin(), castChar);
  return Hash(rBytes);
}
...
Hash Hash::fromBytes(ByteSpan bytes) {
  ...
  std::ranges::transform(bytes, input.begin(), castByte);
