...
concept PodCapable =
    std::semiregular<Type> && std::is_trivially_copyable_v<Type>;
...
template <PodCapable Type> void writePod(std::ostream &out, Type const &value) {
...
void writeOptionalString(std::ostream &out,
                         std::optional<std::string> const &oStr) {
...
template <PodCapable Type> Type readPod(std::ifstream &in) {
  Type value{};
...
void writeToTemp(FilePath const &tempPath, Manifest const &manifest) {
  ...
  if (!out) {
    throw std::runtime_error("Failed to open temporary manifest file: " +
                             tempPath.string());
  }
  ...
  out.flush();
  if (!out) {
    throw std::runtime_error("Failed writing temporary manifest file: " +
                             tempPath.string());
  }
}
...
void fsyncFile(FilePath const &tempPath) {
  ...
  if (!static_cast<bool>(file)) {
    throw std::runtime_error(
        "Failed to reopen temporary manifest file for fsync.");
  }
  ...
  if (fileDescriptor == -1) {
    ...
    throw std::runtime_error("fileno failed on temporary manifest file: " +
                             tempPath.string());
  }
  ...
  if (result != 0) {
    throw std::runtime_error("fsync failed on temporary manifest file: " +
                             tempPath.string());
  }
}
}; // namespace

ManifestStore::ManifestStore(FilePath manifestPath)
...
Manifest ManifestStore::load() const {
  ...
  if (version != kVersion) {
    throw std::runtime_error("Manifest file has unsupported version: " +
...
void ManifestStore::save(Manifest const &manifest) const {
  ...
  if (static_cast<bool>(errorCode)) {
    throw std::runtime_error("Failed to atomically replace manifest file: " +
                             errorCode.message());
  }
}
